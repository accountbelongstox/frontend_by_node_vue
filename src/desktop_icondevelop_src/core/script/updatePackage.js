...
if (dependencies) {
  for (let pkg in dependencies) {
    if (outdatedPackages[pkg]) {
      console.log(`${RED}Updated package (dependencies): ${pkg} from ${dependencies[pkg]} to ^${outdatedPackages[pkg].latest}${RESET}`);
...
if (devDependencies) {
  for (let pkg in devDependencies) {
    if (outdatedPackages[pkg]) {
      console.log(`${RED}Updated package (devDependencies): ${pkg} from ${devDependencies[pkg]} to ^${outdatedPackages[pkg].latest}${RESET}`);
