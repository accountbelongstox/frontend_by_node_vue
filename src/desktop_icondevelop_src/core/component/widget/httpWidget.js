...
class httpWidget {
    ...
    async compareFileSizes(remoteUrl, localPath) {
        if (!fileUnit.isFile(localPath)) return false
        try {
            const remoteSize = await this.getRemoteFileSize(remoteUrl);
            const localSize = fileUnit.getFileSize(localPath);
    ...
    async getFileAndUnzip(url, out, callback, force, del = true) {
        ...
        if (force) {
            if (fileUnit.isFile(downloadFileName)) {
                fileUnit.delete(downloadFileName)
            }
        }
            ...
            if (result.dest) {
                zipWidget.putUnZipTask(
                    ...
                    (usetime) => {
                        ...
                        if (del) fileUnit.delete(result.dest)
    ...
    async getFileAndUnzipOne(url, out, callback, force = true) {
        ...
        if (!winapiWidget.hasListUserData(key, url) && fileUnit.isFile()) {
    ...
    startHTTPServer(startPort) {
        let compile_dir = fileUnit.get_static(`html_compile`)
        let static_dir = fileUnit.get_static()
        let src_dir = fileUnit.get_src_dir()
        let core_dir = fileUnit.get_core_dir()
        let STATIC_DIRS = [
            static_dir,
            compile_dir,
            core_dir,
    ...
    async execEventProcess(category_name, event_name, args, rawData, wsClientFingerprint) {
        if (encyclopedia[category_name] && event_name) {
            if (encyclopedia[category_name][event_name]) {
                let paramNames = utilUnit.getParamNames(encyclopedia[category_name][event_name])
                let trans_args = args.slice();
                let isResult = undefined
                if (utilUnit.isCallByParam(paramNames)) {
                    let callback = (...rArg) => {
                        isResult = true
                        let rData = {
                            data: rArg,
                    ...
                    trans_args = utilUnit.arrangeAccordingToA(paramNames, callback, trans_args)
                    let data
                    if (utilUnit.isAsyncFunction(encyclopedia[category_name][event_name])) {
                ...
                } else if (utilUnit.isPromise(encyclopedia[category_name][event_name])) {
                    ...
                    // AsyncFunction
                } else if (utilUnit.isAsyncFunction(encyclopedia[category_name][event_name])) {
